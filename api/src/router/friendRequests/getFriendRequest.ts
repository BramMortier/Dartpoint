...
export const getFriendRequestHandler: Handler = async (c) => {
    try {
        ...
        return formattedResponse(c, 200, getFriendRequestRoute.responses[200].description, {
            request: request,
        });
    } catch (error) {
        console.error(error);
        return formattedResponse(c, 500, getFriendRequestRoute.responses[500].description);
    }
};
